...
- 🔒 **绝对隐私与离线优先 (100% Local & Offline First)**：所有对话、历史记录与推理计算均在端侧完成，数据永不上传云端。
- ⚡ **原生级端侧加速 (Hardware Accelerated)**：针对桌面 GPU、Apple Neural Engine/Metal 以及移动端 GPU 进行底层优化与显存管理。
- 🧩 **自主智能体与工具生态 (Agent Loop & Tool Runtime)**：具备结构化工具调用（Tool Calling）闭环，支持本地 JavaScript 引擎执行、实时网络分析与搜索。
...
| 能力 | 当前实现 |
| --- | --- |
| 本地大模型 | 加载兼容 LiteRT-LM 的 `.litertlm` 文件；内置 Ministral-3-3B、Gemma 4 4B 下载入口，也支持选择自定义模型 |
...
- 特别感谢以下开源项目与技术生态：
    ...
    - [Compose Rich Editor](https://github.com/MohamedRejeb/Compose-Rich-Editor) — Compose Multiplatform 富文本编辑器
