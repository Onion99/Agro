...
kotlin {
    ...
    listOf(
        ...
        iosTarget.binaries.framework {
            ...
            // isStatic = false？
            // LiteRT archive/dylib paths are passed to the framework linker.
            // Apple SDK framework dependencies are owned by the LiteRT CInterop definition.
...
abstract class ValidateIosLiteRtLmNativeLibsTask : DefaultTask() {
    ...
    fun execute() {
        ...
        val lfsPointers = mutableListOf<String>()
        ...
        requiredDirs.forEach { dir ->
            dylibNames.get().forEach { dylibName ->
                ...
                if (!dylib.isFile) {
                    missing += dylib.path
                } else if (dylib.length() < 1024L &&
                    dylib.readText().startsWith("version https://git-lfs.github.com/spec/v1")
                ) {
                    lfsPointers += dylib.path
                }
            }
        }
        ...
        if (lfsPointers.isNotEmpty()) {
            throw GradleException(
                "Git LFS pointer files found in iOS LiteRT LM dylibs: " +
                    "${lfsPointers.joinToString()}. Run " +
                    "`git -C cpp/lite-rt-lm lfs pull` and rerun the iOS build."
            )
        }
    }
}
