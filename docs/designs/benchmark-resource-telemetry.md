...
| Backend | 计算指标 | 内存指标 |
| --- | --- | --- |
| CPU | 展示真实的 App 进程 CPU 当前值/峰值 | 展示进程内存峰值与正式跑测增量 |
| GPU | 驱动未提供可靠跨平台计数器时显示“系统未提供遥测” | 展示包含主机可见 GPU/native 分配的进程内存；明确不包含设备私有显存 |
| NPU | 驱动未提供可靠跨平台计数器时显示“系统未提供遥测” | 展示包含主机可见 NPU/native 分配的进程内存；明确不包含设备私有加速器内存 |
