...
- Target-specific `validateIosLiteRtLmNativeLibDevice` and `validateIosLiteRtLmNativeLibSimulatorArm64` tasks run on macOS before their matching iOS link tasks and fail early if the required native library is missing. They use a custom task with path and library-name inputs so validation does not capture `Project` from a `doLast` closure under Gradle configuration cache; `validateIosLiteRtLmNativeLibs` remains the aggregate validator.
...
- Each iOS framework link depends only on its matching native archive validator: `iosArm64` builds `ios-device`, while `iosSimulatorArm64` builds `ios-simulator`. The aggregate build/validation tasks remain available for callers that explicitly need the complete target matrix.
...
- After copying the fully linked archive, the Gradle task shuts down the Bazel server from the patched workspace. This releases the Bazel JVM before Kotlin/Native cinterop/framework linking on memory-constrained CI runners; a shutdown failure is diagnostic only and does not invalidate an archive that was already copied.
...
- Common model metadata and generated-audio file access obtain Okio's host file system through an expect/actual boundary. iOS imports the Native `okio.SYSTEM` refinement in `iosMain` rather than resolving `FileSystem.SYSTEM` directly from `commonMain`.
