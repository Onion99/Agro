...
- `LiteRtLmJni` now has an iOS `actual` implementation that calls the LiteRT LM C API for engine creation, conversation creation, synchronous message sending, streaming message sending, cancellation, token count, benchmark metric extraction, and release.
- iOS benchmark info extraction (`getLmConversationBenchmarkInfo`) consumes `litert_lm_conversation_get_benchmark_info`, reads total init time, TTFT, prefill token count and speed, decode token count and speed via `litert_lm_benchmark_info_*` accessors, maps them into common `BenchmarkInfo`, and frees native benchmark memory via `litert_lm_benchmark_info_delete`. The bridge binds `cnames.structs.LiteRtLmBenchmarkInfo` and `com.google.ai.edge.litertlm.cinterop.*`.
